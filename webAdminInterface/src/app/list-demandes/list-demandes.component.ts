...
export class ListDemandesComponent implements OnInit {
  ...
  bread =  [
    ...
    {
      fullPath: "/home/liste des Compagnies",
      ...
      (datas)=>{
        ...
        this.dataSource.forEach(element =>
        {
          if( element.customerId != null)
           ...
           let date =  new Date(element.serverDate) ;

           element.serverDate = date.getDate()+'-'+(date.getMonth()+1)+'-'+date.getFullYear()
        });
  ...
  _confirmCreate(ev)
  {
    ...
    Swal.fire({
      ...
      {
        this.exchange._createDemande(newData)
        .subscribe(
          ()=>{
            Swal.fire({
              icon:'success',
  ...
  _confirmDelete(ev)
  {
    ...
    Swal.fire({
      ...
      if(response.isConfirmed)
      {
        this.exchange._deleteDemande(data._id)
        .subscribe(
          ()=>{
            Swal.fire({
              icon:'success',
  ...
  _confirmUpdate(ev)
  {
    Swal.fire({
      ...
      if(response.isConfirmed)
      {
        this.exchange._updateDemand(ev.newData._id,ev.newData)
        .subscribe(
          ()=>{
            Swal.fire({
              icon:'success',
  ...
  _customAction(ev)
  {
    Swal.fire({
      ...
      {
        this.spinner.show();
        this.exchange._updateDemand(ev.data._id,{ status:1 })
        .subscribe(
          ()=>{
            Swal.fire({
              icon:'success',
