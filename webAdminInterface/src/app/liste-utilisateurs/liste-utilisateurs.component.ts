...
export class ListeUtilisateursComponent implements OnInit {
  ...
  bread =  [
    ...
    },
    {
      ...
      name:"Liste des utilisateurs non actifs&nbsp;>&nbsp;",
      active:true
    }
  ];
  ...
  constructor(
    ...
    this.userType = this.actived.snapshot.params.status;
  ...
  {
    this.spinner.show();
    this.fetchedDemandsData = this.userservice._getUsers("non-actifs")
    .subscribe(
      (datas)=>{
        this.spinner.hide();
  ...
  _activateAccount(event)
  {
    Swal.fire({
      ...
      if(response.isConfirmed)
        this.userservice._illuminateUser(event.data._id,{ isActive:true })
        .subscribe(
          ()=>{
            Swal.fire({
              icon:'success',
